...
static void print_data(FILE *fp, const char *prefix, const char *buf, uint32_t len)
{
	uint32_t g = 0;
	_Bool last_was_newline = 1;
	while(g < len)
	{
		if(last_was_newline)
		{
...
#ifdef DEBUG
		print_data(stderr, YEL "<==" CRESET, conn->out.data + sent, total - sent);
#endif
		assert(n >= 0);
		sent += n;
	}
...
static void generate_response_by_calling_the_callback(context_t *ctx, conn_t *conn, void (*callback)(xh_request*, xh_response*, void*), void *userp)
{
	...
	bool keep_alive;
	{
		...
		else
		{
			...
			if(!strcmp(h_connection, " Keep-Alive") || !strcmp(h_connection, " keep-alive"))
				keep_alive = 1;
	...
	callback(req, res, userp);

#if DEBUG
	if(res->status == 0)
		fprintf(stderr, "The callback did set a status code");
#endif
...
static void when_data_is_ready_to_be_read(context_t *ctx, conn_t *conn, void (*callback)(xh_request*, xh_response*, void*), void *userp)
{
...
#ifdef DEBUG
			print_data(stderr, GRN "==>" CRESET, conn->in.data + conn->in.used, n);
#endif
	...
	while(1)
	{
		...
		if(conn->head_received && conn->body_offset + conn->body_length <= conn->in.used)
		{
			...
			generate_response_by_calling_the_callback(ctx, conn, callback, userp);
	...
	while(!context.exiting)
	{
		...
		for(int i = 0; i < num; i += 1)
		{
			...
			if((events[i].events & (EPOLLIN | EPOLLPRI)) && conn->close_when_uploaded == 0)
			{
				...
				when_data_is_ready_to_be_read(&context, conn, callback, userp);
			}
